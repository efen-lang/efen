...
## Право объявить стратегию

Пакет, которому принадлежит целевой тип, может объявлять стратегии для него без
дополнительного разрешения. Другой пакет может объявить стратегию для этого
типа, только если сам тип явно разрешает внешние стратегии. Точная запись такого
разрешения в объявлении типа ещё не выбрана.

Разрешение относится только к объявлению стратегии. Оно не делает стратегию
видимой, активной или экспортируемой и не выдаёт права изменять HIR чужого типа.
Для generic-цели владельцем считается пакет её номинальной головы: например,
владельцем `Array<User>` остаётся пакет `Array`, а не пакет `User`.
...
активные стратегии одного приоритета, компилятор сообщает конфликт при формировании
