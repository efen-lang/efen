...
Безымянная стратегия активна, когда её объявление видно в текущей области.
Именованная стратегия является альтернативной реализацией и не участвует в
разрешении, пока явно не выбрана через `provide Strategy for Type`.

Если в одной области одновременно видны две действительно пересекающиеся
активные безымянные стратегии, компилятор сообщает конфликт при формировании
области, не дожидаясь конкретного вызова. Порядок объявлений не используется как
правило выбора. Отсутствие стратегии является ошибкой только в точке, где
соответствующее поведение действительно требуется.
