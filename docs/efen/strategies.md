...
Компилятор позволяет применять стратегии к любому типу: классу, структуре,
enum, новому номинальному типу, интерфейсу и другим абстракциям.
...
стратегию к целевому типу.

Объявление внешней библиотеки неизменяемо по умолчанию. `provide` может
применить изменение к внешней цели только при разрешении её владельца; изменение
создаёт рабочий вид для сборки потребителя и не переписывает опубликованный
продукт зависимости. Гранулярность разрешения, транзитивность и синтаксис выдачи
права требуют отдельной спецификации.
...
Именованная и безымянная стратегия активна, когда её объявление видно в текущей
области. `use` импортирует имя или задаёт область, но не создаёт приоритет над
другой уже видимой стратегией.

Если для требуемого вызова, contract или coercion одновременно подходят две
стратегии, компилятор выдаёт ошибку неоднозначности. Порядок объявлений не
используется как правило выбора. Отсутствие стратегии является ошибкой только
в точке, где соответствующее поведение действительно требуется.
...
Видимая стратегия участвует в статическом разрешении автоматически. `use`
управляет импортом и областью видимости. Иногда программисту нужно динамически
