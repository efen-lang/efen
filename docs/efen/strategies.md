...
вызова. Порядок объявлений не используется как правило выбора. Применимый
`StrategySelector`, описанный ниже, получает набор кандидатов до этой
...
Алгоритм выбора расширяется тем же механизмом стратегий. `StrategySelector` —
встроенная compile-time абстракция Efen; стратегия для неё задаёт через `where`,
какие запросы она обслуживает:

```efen
strategy for StrategySelector
where target is Box<let T> {
    // выбор стратегии для Box<T>
}
```

Когда условие подходит, компилятор вызывает эту стратегию вместо встроенного
алгоритма. Сам `StrategySelector` разрешается bootstrap-правилом компилятора,
чтобы его поиск не рекурсировал через себя. Если пользовательский selector не
применим, действует встроенное правило; оно не сравнивает generic-кандидатов по
необъявленной «большей специализации».
