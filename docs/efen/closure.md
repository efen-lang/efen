...
surface-аннотация — `@escaping` в функциональном типе:

```efen
fn register(callback: @escaping () -> Void)
```

`@escaping` разрешает сохранить, вернуть, записать в поле или передать callback
в уходящую task. Без него это ошибка. Origin захватов по-прежнему проверяется
обычными ownership-правилами.
...
`@autoclosure` в функциональном типе `() -> R` позволяет caller передать
выражение `R`; compiler создаёт обычное closure в месте вызова:

```efen
fn assert(condition: @autoclosure () -> Bool, message: String) {
    if !condition() {
        print(message)
    }
}
...
Callee всё равно получает обычное closure, поэтому modifier сохраняется и при
косвенном вызове. Сочетание `@autoclosure @escaping` запрещено: явное closure
точно выражает намерение сохранить захваты.
