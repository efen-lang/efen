...
Каноническую форму выбирает обязательное правило стиля. Объявления `param`
обязательны, если runtime-параметров больше трёх, у какого-нибудь параметра есть
значение по умолчанию или есть pack `...values`; иначе канонична заголовочная
форма. Отклонение в любую сторону получает диагностику стиля
`S.function-parameter-layout`, по умолчанию ошибку. Формы не смешиваются: при
`param` в теле круглых скобок параметров в заголовке нет. Variadic-параметр
считается одним; generic-параметры и неявный receiver не считаются. Общая модель style diagnostics
