...
| **Layout** | Декларативное описание структур, memory descriptors, их размещения, связей и условий. `Layout<T>` как generic-тип является отдельной открытой темой. | [Layout и дескрипторы](memory/addresses.md), [примеры](memory/layout-examples/) |
...
| **Population / население** | Живые элементы конкретного memory descriptor и layout-instance; descriptor задаёт origin и допустимые операции памяти. | [Layout и дескрипторы](memory/addresses.md) |
