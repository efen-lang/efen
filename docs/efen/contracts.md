...
локальной переменной, параметра или результата. Неизвестное конкретное значение
с runtime-полиморфизмом выражается интерфейсом; contract используется только в
ограничениях и `conforms`.
