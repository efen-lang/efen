...
Внутри тела `match` каждая строка на верхнем уровне скобок начинает новую ветку.
Правило продолжения строки, начинающейся с `.` или с бинарного оператора, здесь
не действует, поэтому `.south:` и `..<0:` открывают ветку. Многострочное тело
ветки записывается в `{ }`.
