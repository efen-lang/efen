...
> Граница проектирования: для этой формы ещё не определены точка продолжения
> после обработанного исключения и то, входят ли исключения из тела обработчика
> в область следующих `catch`/`on`. До решения этих двух вопросов frontend не
> должен принимать `catch` без `try` как полностью специфицированную конструкцию.
