...
`catch` он не ловит. После обработанного исключения выполнение продолжается с
первой инструкции после блока обработчика; прерванный код выше него не
возобновляется. Исключение, брошенное телом `catch`, не попадает в следующие
ниже `catch` или `on`, а распространяется во внешнюю область. Переменные,
объявленные между `catch`, видны дальше по блоку.
