...
> Статус: этот документ содержит прежнюю высокоуровневую модель и требует
> переписывания. В текущей модели пользовательский деструктор класса и
> низкоуровневый `Disposable` различны: `Disposable` non-throwing и выполняется
> после пользовательского деструктора. Когда исчезает последний владелец,
> compiler запускает пользовательский деструктор только внутри неявной
> `clean`-области. Обычный бросающий destructor вызывается только явно автором.
> Старые примеры ниже не устанавливают обратную семантику.
