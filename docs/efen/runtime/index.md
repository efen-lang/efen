...
Runtime API предоставляет низкоуровневый доступ, который может нарушить safety гарантии языка:

```efen
import runtime.unsafe

// Небезопасное приведение типов
let ptr = UnsafePointer<Int>(address)
let value = ptr.pointee

// Небезопасное изменение памяти
let mutablePtr = UnsafeMutablePointer<Int>(address)
mutablePtr.pointee = 42

// Небезопасный доступ к raw байтам
let bytes = unsafeBitCast(value, to: [UInt8].self)
```

⚠️ **Предупреждение**: использование unsafe API требует глубокого понимания и может привести к undefined behavior.
...
2. **Изолируйте unsafe код**
   - Оборачивайте unsafe API в безопасные абстракции
   - Документируйте invariants

3. **Проверяйте платформу**
   - Используйте условную компиляцию для платформо-зависимого кода
   - Предоставляйте fallback реализации

4. **Тестируйте в разных режимах**
...
- Unsafe API требует явного opt-in
