...
> Этот документ исследует колоночное хранение до принятия текущей модели
> дескрипторов памяти. Формы `source`, `storage`, старые правила `set` и
> разнесение встроенных компонентов не являются текущим синтаксисом исходного
> языка. Нормативная основа: [Компоновка и дескрипторы памяти](addresses.md).
> Семантику колоночных дескрипторов предстоит перенести отдельно, сохранив
> описанные ниже гарантии идентичности и отображений.
