...
> Этот документ исследует columnar storage до принятия текущей модели memory
> descriptors. Формы `source`, `storage`, старые правила `set` и разнесение
> embedded-компонентов не являются текущим surface syntax. Нормативная основа:
> [Layout и дескрипторы памяти](addresses.md). Семантику columnar descriptors
> предстоит перенести отдельно, сохранив identity и mapping guarantees ниже.
