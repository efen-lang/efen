...
> Исследование форм хранения предшествует текущему синтаксису memory
> descriptors. Примеры `storage` ниже не являются нормативным surface syntax.
> Текущая основа задана в [Layout и дескрипторы памяти](addresses.md).
