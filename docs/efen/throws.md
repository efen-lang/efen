...
поэтому `try` вокруг регистрации ничего не ловит. Получатель выражает это право
модификатором `@escaping` в функциональном типе:

```efen
fn wrongForStoredCallback(register: (@escaping () -> Void) -> Void) {
    try {
