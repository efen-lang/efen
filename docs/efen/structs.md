...
`Function` не становится подтипом `BaseNode` и не получает vtable. Голое имя
типа физически встраивает его в объявленную позицию: поскольку `BaseNode` стоит
первым, обычное значение `Function` начинается со структуры `BaseNode`.
Выражение `function.BaseNode` обозначает встроенное место целиком.

Storage projection населения может хранить логические компоненты в отдельных
columns, не материализуя обычное row-значение `Function`. Получение целого
`Function` тогда обязано собрать значение с тем же порядком встроенных
компонентов. См. [колоночные layout](memory/columnar-layouts.md).
